...
예를 들어, 총 5명이 있고,
 P1 = 3, P2 = 1, P3 = 4, P4 = 3, P5 = 2
  인 경우를 생각해보자. [1, 2, 3, 4, 5]
   순서로 줄을 선다면, 1번 사람은 3분만에
    돈을 뽑을 수 있다. 2번 사람은 1번 사람이 돈을
     뽑을 때 까지 기다려야 하기 때문에, 3+1 = 4분이
      걸리게 된다. 3번 사람은 1번, 2번 사람이
       돈을 뽑을 때까지 기다려야 하기 때문에, 
       총 3+1+4 = 8분이 필요하게 된다.
...
줄을 [2, 5, 1, 4, 3] 순서로 줄을 서면,
 ...
 1번 사람은 1+2+3 = 6분, 4번 사람은 1+2+3+3 = 9분,
  ...
  각 사람이 돈을 인출하는데 필요한 시간의 합은
   1+3+6+9+13 = 32분이다. 이 방법보다 더 필요한 
   시간의 합을 최소로 만들 수는 없다.

줄을 서 있는 사람의 수 N과 각 
사람이 돈을 인출하는데 걸리는 시간 Pi가 주어졌을 때, 
...
class ATM:
    def __init__(self):
        self.n = int(input())
        self.personlist=list(map(int,input().split()))
        self.personlist.sort()
        for i in range(1,self.n):
            self.personlist[i]+=self.personlist[i-1]
        print(sum(self.personlist))
a=ATM()